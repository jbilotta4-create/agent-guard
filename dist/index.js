...
function detectLoop(sessionId, loopWindowMs, loopThreshold, maxConsecutiveErrors) {
    ...
    for (const a of windowActions) {
        if (a.resultNontrivial !== undefined) {
    ...
    const writeActions = windowActions.filter(a => (a.toolName === "write" || a.toolName === "edit") && !a.isError);
    // Group by file path (extract from paramsHash — paths are part of the hash)
    const writeFileCounts = new Map();
...
function recordAction(sessionId, toolName, params, isError, resultNontrivial) {
    const now = Date.now();
    const record = {
        toolName,
...
function matchesRule(toolName, rule) {
    if (Array.isArray(rule.toolName)) {
        return rule.toolName.includes(toolName);
    }
    return rule.toolName === toolName;
}
function verifyState(toolName, params, result, isError, rule) {
    const targetPath = getNestedValue(params, rule.paramPath);
        ...
        case "file_exists": {
            ...
            try {
                ...
                return {
                    passed: false,
                    rule,
                    detail: `Error checking file ${targetPath}: ${err.message}`,
        ...
        case "content_match": {
            ...
            try {
                ...
                return {
                    passed: false,
                    rule,
                    detail: `Error checking content of ${targetPath}: ${err.message}`,
                    actualState: "check_error",
                };
            }
        }
        ...
        api.on("after_tool_call", async (event, ctx) => {
            ...
            if (loopResult.isLoop) {
                ...
                if (now - lastAlert > 300000) {
                    api.logger.warn?.(`Agent Guard: LOOP DETECTED [${loopResult.loopType}] session=${sessionId} tool=${toolName} repeats=${loopResult.repeatedActions} severity=${loopResult.severity} confidence=${loopResult.confidence.toFixed(2)} shouldStop=${loopResult.shouldStop}`);
            ...
            if (config.stateVerification) {
                ...
                const matchingRules = rules.filter((r) => matchesRule(toolName, r));
                for (const rule of matchingRules) {
                    ...
                    if (!verifyResult.passed) {
                        ...
                        api.logger[shouldBlock ? "error" : "warn"]?.(`Agent Guard: STATE VERIFICATION FAILED [${rule.checkType}] tool=${toolName} severity=${rule.severity} consecutive_failures=${failCount} detail=${verifyResult.detail}`);
                        if (shouldBlock) {
                        ...
                        if (config.logLevel === "debug") {
                            api.logger.info?.(`Agent Guard: State verification passed [${rule.checkType}] tool=${toolName} detail=${verifyResult.detail}`);
                        }
                    }
                }
            }
        ...
        api.on("before_tool_call", async (event, ctx) => {
            ...
            if (config.stateVerification) {
                ...
                if (verifyBlockTime && Date.now() - verifyBlockTime < 300000) {
                    ...
                    api.logger.warn?.(`Agent Guard: BLOCKING tool call [${event.toolName}] — state verification failed ${failCount} consecutive times`);
                    return {
                        block: true,
